                    ...
                    <form onSubmit={e=>this.login(e)}>
                        <div className="input-area">
                            <input ref={input=>this.id = input} className="id" placeholder="아이디" type="text"
                                   ...
                                   value={this.state.stored_id}/>
                            <input ref={input=>this.pw = input} className="pw" placeholder="비밀번호"
                                   onChange={e=>this.setState({stored_pw: e.target.value})} value={this.state.stored_pw}
                                   type="password"/>
                            <label htmlFor="viewMode" style={{fontSize: '15px'}}>
                                <input onChange={e=>this.setState({viewMode: !this.state.viewMode})} id="viewMode"
                                       type="checkbox" value="Bike"/>
                                뷰모드
                            </label>
                            <label htmlFor="save" style={{fontSize: '15px'}}>
                                <input onChange={e=>this.setState({saveMode: !this.state.saveMode})} id="save"
                                       type="checkbox"/>
                                아이디/비번 저장
                            </label>
                        </div>
