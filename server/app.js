...
app.post('/item', upload.array('image'), function (req, res) {
    ...
    if (req.query.id) {
        db.query('insert into Item ' +
            ' (type, owner, title, location, produced_area, real_area, floor, total_floor, room, toilet, specification, available, j_price, m_price, b_price, w_price, y_price)' +
            'values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [data.type, req.query.id, data.title, data.location, data.produced_area, data.real_area, data.floor, data.total_floor, data.room, data.toilet, data.specification, data.available, data.j_price || null, data.m_price || null, data.b_price || null, data.w_price || null, data.y_price || null], function (err, result) {
...
app.put('/user', upload.array('image'), function (request, res) {
    ...
    db.query('update User set store_name=?, ceo_name=?, login_id=?, login_pw=?, tel=?, phone=?, video_num=?, image_num=? ,contract_start=?, contract_duration=?, contract_end=?, video_id=?, image_url=?, message=? where id=?', [body.store_name, body.ceo_name, body.login_id, body.login_pw, body.tel, body.phone, body.video_num, body.image_num, body.contract_start, body.contract_duration, body.contract_end, body.video_id, filename, body.message, request.query.id], function (err, result) {
        res.json({result: true});
    });
});
